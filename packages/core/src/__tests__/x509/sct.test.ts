...
describe('SignedCertificateTimestamp', () => {
  ...
  const sctBuffer = hexToUint8Array(
    '00' + // version
      logID + // logID
      timestamp + // timestamp
  ...
  describe('#parse', () => {
    describe('when the SCT is valid', () => {
      it('parses correctly', () => {
        ...
        expect(sct.logID).toStrictEqual(hexToUint8Array(logID));
        expect(sct.timestamp).toStrictEqual(hexToUint8Array(timestamp));
        expect(sct.extensions).toHaveLength(0);
        expect(sct.hashAlgorithm).toEqual(0x04);
        expect(sct.signatureAlgorithm).toEqual(0x03);
        expect(sct.signature).toStrictEqual(hexToUint8Array(signature));
      });
    });
    ...
    describe('when the SCT is invalid', () => {
      describe('when the SCT buffer is too short', () => {
        it('throws an error', () => {
          expect(() =>
            SignedCertificateTimestamp.parse(hexToUint8Array(''))
